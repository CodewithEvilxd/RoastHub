...
import { useRoastHub, type Resume } from "@/lib/store";
...
export default function ProfilePage() {
  ...
  const { currentUser, byOwner } = useRoastHub();
  ...
  return (
    <div className="grid gap-6">
      ...
      <ComicCard className="flex items-center gap-4 p-6">
        <div className="relative h-20 w-20 overflow-hidden rounded-full border-4 border-[#2c2c2c] bg-white shadow-[4px_4px_0_#2c2c2c]">
          <Image
          ...
          {session && (
            <div className="mt-3 flex gap-4 text-sm flex-wrap">
              <span
                className={cn(
                  display.className,
                  "bg-[#F2D5A3] px-3 py-1 flex items-center gap-2 rounded-full border-2 border-[#2c2c2c]  font-bold"
                )}
              >
              ...
              <button
                ...
                className={cn(
                  display.className,
                  "flex items-center gap-1 rounded-full border-2 sm:border-[3px] border-[#2c2c2c] bg-red-400 hover:bg-red-500 px-2 sm:px-3 py-1 sm:py-1.5 text-xs sm:text-sm font-normal shadow-[2px_2px_0_#2c2c2c] sm:shadow-[3px_3px_0_#2c2c2c] hover:-translate-y-1 transition-all text-[#2c2c2c] disabled:opacity-50 disabled:cursor-not-allowed"
                )}
              >
